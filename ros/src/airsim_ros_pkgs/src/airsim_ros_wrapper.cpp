...
void AirsimROSWrapper::create_ros_pubs_from_settings_json()
{
    ...
    gimbal_angle_euler_cmd_jbs_sub_ = nh_private_.subscribe("gimbal_angle_euler_cmd_jbs",50,&AirsimROSWrapper::gimbal_angle_euler_jbs_cmd_cb,this);
    ...
    for (const auto& curr_vehicle_elem : AirSimSettings::singleton().vehicles)
    {
        ...
        vehicle_ros->pose_local_enu_pub = nh_private_.advertise<geometry_msgs::PoseStamped>(curr_vehicle_name + "/pose_local_enu", 10);
...
void AirsimROSWrapper::gimbal_angle_euler_jbs_cmd_cb(const geometry_msgs::Point &rpy) {
    tf2::Quaternion quat_control_cmd;
    quat_control_cmd.setRPY(rpy.x,-rpy.y,-rpy.z);
    quat_control_cmd.normalize();


    gimbal_cmd_.target_quat = get_airlib_quat(quat_control_cmd);
    gimbal_cmd_.camera_name = "front_left_custom";
    gimbal_cmd_.vehicle_name = "SimpleFlight";
...
void AirsimROSWrapper::publish_vehicle_state()
{
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_)
    {
        ...
        vehicle_ros->pose_local_enu_pub.publish(vehicle_ros->curr_pose_enu);
...
//                              << " , " << pose_true.position.z());
            ...
            if (not std::isnan(pose_true.position.x())) {
                ...
                object_tf.header.frame_id = "world_enu";
...
void AirsimROSWrapper::update_commands()
{
    ...
    {
        std::lock_guard<std::mutex> guard(drone_control_mutex_);
        airsim_client_->simSetCameraPose(gimbal_cmd_.camera_name, get_airlib_pose(0.55, 0, 0.3, gimbal_cmd_.target_quat), gimbal_cmd_.vehicle_name);
        airsim_client_->simSetCameraPose("infrared", get_airlib_pose(0.55, 0, 0.3, gimbal_cmd_.target_quat), gimbal_cmd_.vehicle_name);
    }
...
//    ROS_INFO("cam to body? [%f, %f, %f] "  , cam_tf_body_msg.transform.translation.x ,
